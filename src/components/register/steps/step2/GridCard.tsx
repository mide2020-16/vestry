...
export default function GridCard({ mesh, ticketType, isSelected, onSelect }: GridCardProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
    ...
    >
      ...
      <div className="relative w-full h-28 rounded-xl mb-2.5 overflow-hidden bg-white/5">
        <Image src={mesh.image_url} alt={mesh.name} fill sizes="50vw" className="object-cover" />
        {mesh.modelUrl && (
      ...
      <p className={`font-semibold text-sm leading-tight ${isSelected ? 'text-amber-400' : 'text-white'}`}>
        {mesh.name}
      </p>

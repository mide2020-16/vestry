...
export function NavItem({
  ...
  return (
    ...
    >
      ...
      {/* Active dot — expanded */}
      {isActive && !collapsed && (
        <span className="ml-auto w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0 animate-pulse" />
      )}

      {/* Active bar — collapsed */}
      {isActive && collapsed && (
        <span className="absolute -left-2 w-0.5 h-5 bg-amber-400 rounded-r-full" />
      )}
    </Link>
  );
}

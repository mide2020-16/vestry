  ...
  return (
    <div className="max-w-md border bg-white/7 hover:border-gray-100 rounded-2xl p-8">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        >
          <BellIcon
            className={`w-4 h-4 ${subscription ? "text-green-600" : "text-gray-400"}`}
          />
        </div>
        <div className="flex-1">
          <p className="text-sm font-bold text-gray-50">Push notifications</p>
          <p
            className={`text-xs ${subscription ? "text-green-600" : "text-gray-400"}`}
          >
            {subscription ? "Active" : "Off"}
          </p>
        </div>
        <div
          className={`w-2 h-2 rounded-full ${subscription ? "bg-green-500" : "bg-gray-300"}`}
        />
      </div>
      ...
      {subscription ? (
        ...
        <div className="space-y-3">
          <p className="text-xs text-gray-500 leading-relaxed">
            Stay updated with event reminders, announcements, and check-in
            alerts.
          </p>
          <button
            type="button"

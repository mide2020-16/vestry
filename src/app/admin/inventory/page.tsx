...
function getColorLabel(hex: string, colors: { label: string; value: string }[]): string {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm text-neutral-400">
          ...
          <tbody className="divide-y divide-neutral-800/50">
            {rows.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="text-center py-8 text-neutral-600 italic text-xs">
              ...
              rows.map((stat) => (
                <tr key={stat.name} className="hover:bg-neutral-800/20 transition-colors group">
                  {columns.map((col) => (
                    <td key={col.label} className={`px-4 py-3 align-top ${col.align ?? ''}`}>
                      {col.render(stat)}
                    </td>
                  ))}
                </tr>
...
const meshColors: { label: string; value: string }[] = settingsDoc?.meshColors ?? [];
    ...
    if (reg.meshSelection) {
      ...
      if (stat) {
        ...
        if (reg.meshColor || reg.meshSize) {
          ...
          if (existing) {
            existing.qty += qty;
            if (reg.meshInscriptions) existing.inscriptions.push(reg.meshInscriptions);
          } else {
  ...
  return (
    <div className="space-y-8 p-8">
      <div>
        ...
        <p className="text-neutral-500 mt-2 text-sm">Quantities calculated from successful payments only.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 items-start">
        <InventoryTable
          ...
          columns={[
            {
              label: 'Item',
              render: (s) => <span className="font-bold text-white block">{s.name}</span>,
            },
            {
              ...
              render: (s) => (
                <div className="flex flex-col gap-4">
                  {s.colorSizes && s.colorSizes.length > 0 ? (
                    s.colorSizes.map((cs) => (
                      <div key={`${cs.color}-${cs.size}`} className="space-y-2 border-l border-neutral-800 pl-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-neutral-300 text-[11px] font-semibold uppercase tracking-tight">
                            {cs.color} <span className="text-neutral-600 mx-0.5">|</span> {cs.size}
                          </span>
                        ...
                        {cs.inscriptions.length > 0 && (
                          <div className="flex flex-wrap gap-1">
                            {cs.inscriptions.map((text, idx) => (
                              <span key={idx} className="bg-neutral-800 text-amber-500/80 text-[9px] px-1.5 py-0.5 rounded border border-amber-500/20 italic">
                                &quot;{text}&quot;
                              </span>
                            ))}
                          </div>
            ...
            {
              label: 'Total Qty',
              render: (s) => <span className="font-black text-white text-lg">{s.count}</span>,
              align: 'text-right',
            },
            {
              label: 'Revenue',
              render: (s) => <span className="font-mono text-neutral-400 text-xs tabular-nums">₦{s.revenue.toLocaleString()}</span>,
              align: 'text-right',
            },
          ]}
        />
        ...
        <InventoryTable
          ...
          columns={[
            {
              label: 'Item Name',
              render: (s) => <span className="font-bold text-white">{s.name}</span>,
            },
            {
              label: 'Category',
              render: (s) => <span className="uppercase text-[10px] font-black tracking-widest text-neutral-600">{s.category}</span>,
            },
            {
              label: 'Count',
              render: (s) => <span className="font-black text-emerald-400 text-lg">{s.count}</span>,
              align: 'text-right',
            },
          ]}
        />

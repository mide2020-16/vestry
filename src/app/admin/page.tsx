...
} from "@/components/admin/DownloadButton";
...
export default async function AdminDashboardPage() {
  ...
  const singleTickets = registrations.filter(
    (r) => r.ticketType === "single",
  ).length;
  const coupleTickets = registrations.filter(
    (r) => r.ticketType === "couple",
  ).length;
  ...
  const registrationsForPDF: RegistrationForPDF[] = paidRegistrations.map(
    (r) => ({
      _id: r._id.toString(),
      name: r.name,
      email: r.email,
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        <StatCard
          label="Total Revenue"
          accentColor="bg-amber-500/5 border-amber-500/10"
        >
        ...
        <StatCard
          ...
          accentColor="bg-emerald-500/5 border-emerald-500/10"
        >
        ...
        <StatCard
          label="Ticket Types"
          accentColor="bg-blue-500/5 border-blue-500/10"
        >
          <SplitStat
        ...
        <StatCard
          ...
          accentColor="bg-purple-500/5 border-purple-500/10"
        >
          <SplitStat
            left={{
